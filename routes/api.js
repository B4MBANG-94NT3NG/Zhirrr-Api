...
loghandler = {
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey invalid, gak punya apikey? chat gw aja yaaa di wa.me/6282313021398 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
    ...
    error: {
        status: true,
        creator: `${creator}`,
        message: 'apikey Benar gunakan dengan baik mumpung unlimited'
    }
}
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'BambangGans') return res.json(loghandler.invalidKey)
...
router.get('/genre/action', (req, res) => {
...
Axios.get('https://kartun-api-zahirr.herokuapp.com/api/genre/action')
.then(({ data }) => {
...
Axios.get('https://kartun-api-zahirr.herokuapp.com/api/genre/adventure')
.then(({ data }) => {
     const $ = cheerio.load(data)
     let title = []
     let url = []
...
router.get('/genre/movie', (req, res) => {
var apikeyInput = req.query.apikey
...
Axios.get('https://kartun-api-zahirr.herokuapp.com/api/genre/movie')
.then(({ data }) => {
     const $ = cheerio.load(data)
     let title = []
     let url = []
...
Axios.get('https://kartun-api-zahirr.herokuapp.com/api/genre/comedy')
.then(({ data }) => {
     const $ = cheerio.load(data)
     let title = []
     let url = []
...
Axios.get('https://kartun-api-zahirr.herokuapp.com/api/genre/drama')
.then(({ data }) => {
     const $ = cheerio.load(data)
     let title = []
     let url = []
...
router.get('/genre/military', (req, res) => {
var apikeyInput = req.query.apikey
...
Axios.get('https://kartun-api-zahirr.herokuapp.com/api/genre/military')
.then(({ data }) => {
     const $ = cheerio.load(data)
     let title = []
     let url = []
...
router.get('/kartun/search', (req, res) => {
var apikeyInput = req.query.apikey
...
Axios.get('https://kartun-api-zahirr.herokuapp.com/api/kartun/search?film=spongebob')
.then(({ data }) => {
     const $ = cheerio.load(data)
     let title = []
     let url = []
